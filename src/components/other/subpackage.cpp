...
void SubPackage::slotSave()
{
    ...
    // No native Save dialog on WASM — savePackage() will hand the bytes to
    // the browser as a download.
    QString defName = QFileInfo( pkgeFile ).fileName();
    if( defName.isEmpty() ) defName = m_name + ".package";
    savePackage( defName );
#else
...
{
#ifdef __EMSCRIPTEN__
    QFileDialog::getOpenFileContent( tr("Packages (*.package);;All files (*.*)"),
        [this]( const QString& fileName, const QByteArray& content )
        {
            ...
            Circuit::self()->saveCompChange( m_id, "Package_File", tmp );

            QDir pdir = QFileInfo( Circuit::self()->getFilePath() ).absoluteDir();
            QString pkgeFile = pdir.relativeFilePath( tmp );
...
QString SubPackage::packageToString()
{
    QString out;
    ...
    for( Pin* pin : m_pkgePins ) out += pinEntry( pin );
...
void SubPackage::savePackage( QString fileName )
{
    ...
    QString contents = packageToString();

#ifdef __EMSCRIPTEN__
    QFileDialog::saveFileContent( contents.toUtf8(), QFileInfo( fileName ).fileName() );
#else
    QFile file( fileName );
